...
// Use puppeteer-extra with stealth and adblocker plugins for anti-detection
import vanillaPuppeteer from 'puppeteer';
import { addExtra } from 'puppeteer-extra';
import StealthPlugin from 'puppeteer-extra-plugin-stealth';
import * as AdblockerPluginModule from 'puppeteer-extra-plugin-adblocker';
import type { Browser, Page } from "puppeteer";

// Handle ESM/CJS default export compatibility for AdblockerPlugin
const AdblockerPlugin = (AdblockerPluginModule as any).default || AdblockerPluginModule;

// Create puppeteer-extra instance (official recommended for TypeScript)
const puppeteer = addExtra(vanillaPuppeteer as any);

// Configure Stealth plugin with all evasions (official recommended)
const stealth = StealthPlugin();
puppeteer.use(stealth);

// Add Adblocker plugin to block ads and trackers (saves bandwidth, improves speed)
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        quality: { type: "number", description: "JPEG quality 0-100 (default: 90, only for jpeg)" },
        format: { type: "string", description: "Image format: 'png' (default, lossless) or 'jpeg' (smaller)" },
      },
      required: ["name"],
    },
  },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        cookies: {
          type: "array",
          description: "Array of cookie objects with name, value, domain, path properties",
          items: {
            type: "object",
            properties: {
...
async function ensureBrowser({ launchOptions, allowDangerous }: any) {
  ...
  if (!browser) {
    // Anti-detection browser args (official recommended minimal set)
    const stealthArgs = [
    ...
    const docker_args = { headless: true, args: ["--no-sandbox", "--disable-setuid-sandbox", "--single-process", "--no-zygote", ...stealthArgs] }
...
async function handleToolCall(name: string, args: any): Promise<CallToolResult> {
  ...
  switch (name) {
    ...
    case "puppeteer_screenshot": {
      ...
      // PNG by default for clarity (lossless), JPEG optional for speed
      const format = args.format === 'jpeg' ? 'jpeg' : 'png';
      const quality = args.quality ?? 90; // Only used for JPEG
      
      const screenshotOptions: any = {
        encoding: "base64",
        fullPage: false,
        type: format,
        ...(format === 'jpeg' && { quality }), // quality only applies to JPEG
      };

      const screenshot = await (args.selector ?
        (await page.$(args.selector))?.screenshot(screenshotOptions) :
      ...
      const formatInfo = format === 'jpeg' ? `jpeg, q:${quality}` : 'png';
      return {
        content: [
          {
            type: "text",
            text: `Screenshot '${args.name}' taken at ${width}x${height} (${formatInfo})`,
          } as TextContent,
          {
            type: "image",
            data: screenshot,
    ...
    case "puppeteer_set_cookie":
      try {
        const cookiesToSet = args.cookies.map((cookie: any) => ({
          name: cookie.name,
          value: cookie.value,
        ...
        return {
          content: [{
            type: "text",
            text: `Set ${cookiesToSet.length} cookie(s) successfully`,
          }],
          isError: false,
        };
      } catch (error) {
        return {
          content: [{
            type: "text",
            text: `Failed to set cookies: ${(error as Error).message}`,
          }],
          isError: true,
        };
      }
